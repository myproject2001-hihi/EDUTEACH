...
export function MinesweeperGame({ questions, onClose, isStudentMode = false, onSubmitWork }: MinesweeperGameProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full min-h-[500px] w-full text-slate-100 select-none overflow-hidden relative">
      
      {/* LEFT: QUESTION AND SCOREBOARD PANEL */}
      <div className="w-full lg:w-80 bg-slate-900/95 border-b lg:border-b-0 lg:border-r border-slate-800 p-4 sm:p-5 flex flex-col justify-between shrink-0 relative z-30 shadow-2xl">
        <div className="space-y-4">
          <button 
            onClick={onClose}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-xs font-bold text-slate-200 rounded-lg transition"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Quay lại
          </button>

          <div className="bg-slate-950/80 border border-slate-800 p-4 rounded-2xl shadow-inner relative overflow-hidden min-h-[220px] flex flex-col justify-between">
            {isGameOver ? (
              <div className="text-center space-y-4 py-6 animate-scaleIn flex-1 flex flex-col items-center justify-center">
                <div className="w-12 h-12 bg-emerald-500/10 border border-emerald-400 rounded-full flex items-center justify-center text-emerald-400 text-xl font-bold animate-bounce">
                  🏆
                </div>
                <h4 className="text-lg font-black text-white">Hoàn Thành!</h4>
                ...
                <button 
                  onClick={restartGame}
                  className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs rounded-xl active:scale-95 transition-all"
                >
                  Chơi lại
                </button>
              </div>
              ...
              <>
                <div className="text-center border-b border-slate-800/60 pb-2">
                  <span className="text-emerald-400 font-extrabold text-[10px] tracking-widest uppercase">
                    Câu hỏi {currentQuestionIndex + 1} / {gameQuestions.length}
                  </span>
                </div>

                <div className="flex-1 flex items-center justify-center py-4">
                  <div className="text-sm sm:text-base font-extrabold text-slate-100 text-center leading-relaxed">
                    <MarkdownMath content={q?.question || 'Câu hỏi...'} />
                  </div>
                </div>

                <div className="text-[10px] text-slate-500 flex items-center gap-1 justify-center border-t border-slate-800/60 pt-2 font-medium">
                  <Keyboard className="w-3.5 h-3.5" /> Sử dụng WASD / Phím mũi tên để di chuyển
                </div>
              </>
            )}
          </div>
        ...
        <div className="mt-4 p-3 bg-indigo-950/20 border border-indigo-900/30 rounded-xl space-y-2 hidden lg:block">
          <span className="text-[9px] font-black uppercase text-indigo-400 tracking-wider">
            Cách phá mìn an toàn:
          </span>
          <p className="text-[10px] text-slate-400 leading-normal">
            Nhấp chuột trực tiếp lên các ô nhãn chữ <span className="font-bold text-slate-200">A, B, C, D</span> trên bãi cỏ để lính rà mìn tự động đi tới và gỡ mìn tương ứng!
          </p>
        </div>
      </div>

      {/* RIGHT: INTERACTIVE MILITARY GRASS MAP */}
      <div className="flex-1 h-[400px] lg:h-auto relative bg-slate-950 overflow-hidden" id="game-container">
        
        {/* Float Status Ribbon */}
        <div className="absolute top-4 left-0 right-0 z-20 flex justify-center pointer-events-none">
          <div className="bg-slate-900/90 text-white px-5 py-1.5 rounded-full border border-slate-800 shadow-xl flex items-center gap-4 pointer-events-auto">
            <div className="flex items-center gap-1.5">
              <Star className="w-4 h-4 text-amber-400 fill-amber-400 animate-spin-slow" />
              <span className="font-black text-lg text-amber-400">{score}</span>
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Điểm</span>
            </div>
        ...
        <div 
          ...
          style={{
            backgroundColor: '#4d822d',
            backgroundImage: `
              radial-gradient(#5a9438 15%, transparent 16%),
              radial-gradient(#5a9438 15%, transparent 16%)
            `,
        ...
        >
          ...
          {!isGameOver && currentAnswers.map((ans, idx) => (
            ...
            >
              ...
              <div className="relative flex flex-col items-center">
                <div className="w-10 h-10 rounded-full border-2 border-white flex items-center justify-center text-white font-black text-sm shadow-2xl transition-all duration-200 bg-slate-900 group-hover:scale-110">
                  {ans.displayId}
                </div>
                
                {/* Answer Banner tag */}
                <div className="mt-1.5 px-2.5 py-0.5 bg-slate-950/80 border border-slate-850 rounded-md text-[10px] font-bold text-slate-200 shadow-md whitespace-nowrap group-hover:bg-slate-900 transition-all">
          ...
          >
            <div className="relative w-12 h-12 flex flex-col items-center justify-center">
              {/* Helmet cap circle */}
              <div className="absolute top-0 w-10 h-10 bg-emerald-800 rounded-full border-2 border-emerald-950 shadow-2xl z-20 flex items-center justify-center">
              ...
              <div className="absolute top-1.5 w-12 h-7 bg-emerald-900 rounded-full border border-emerald-950 z-10" />
              {/* Backpack bag */}
              <div className="absolute bottom-0 w-8 h-4 bg-amber-800 rounded-md border border-amber-950 z-0" />
              {/* Torso */}
              <div className="absolute top-4 w-9 h-6 bg-emerald-700 rounded-xl border border-emerald-950 z-10" />
            </div>
          </div>
