  ...
  return (
    <div className="fixed inset-0 z-[10000] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-2 sm:p-6">
      <div className="bg-slate-900 text-slate-100 w-full max-w-4xl h-full max-h-[92vh] sm:max-h-[88vh] rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col relative border border-slate-700">
        ...
        <div className="flex-1 p-4 sm:p-6 flex flex-col justify-between overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center justify-between gap-3 mb-3 shrink-0">
            ...
            <span className="text-[11px] text-slate-400 italic">
              {isFlipped ? '🔄 Đang xem mặt sau' : '👆 Click thẻ để lật'}
            </span>
          </div>

          {/* Empty State vs Interactive Card */}
          {displayCards.length === 0 || (!activeCard.front.trim() && !activeCard.back.trim()) ? (
            <div className="flex-1 bg-slate-800/60 border-2 border-dashed border-slate-700 rounded-3xl p-8 flex flex-col items-center justify-center text-center space-y-4 my-auto">
            ...
            <div className="flex-1 my-2 flex items-center justify-center">
              <div 
                ...
                className="w-full max-w-2xl h-64 sm:h-80 md:h-96 perspective-1000 cursor-pointer group relative"
              >
                <div 
                ...
                >
                  {/* FRONT SIDE */}
                  <div className="absolute w-full h-full backface-hidden bg-gradient-to-b from-slate-800 to-slate-900 border-2 border-indigo-500/40 group-hover:border-indigo-500 rounded-3xl shadow-2xl flex flex-col justify-between p-6 sm:p-8 transition-colors overflow-hidden">
                    {/* Top Tag */}
                    <div className="flex items-center justify-between border-b border-slate-700/60 pb-3">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 rounded-full text-xs font-bold uppercase tracking-wider">
                        <Sparkles className="w-3.5 h-3.5" /> Mặt trước (Câu hỏi / Từ vựng)
                      </span>
                    ...
                    <div className="flex-1 flex items-center justify-center text-center py-4 px-2 overflow-y-auto custom-scrollbar">
                      <div className="text-lg sm:text-2xl font-bold text-slate-100 leading-relaxed">
                        <MarkdownMath content={activeCard.front || '(Chưa nhập nội dung mặt trước)'} />
                      </div>
                    </div>

                    {/* Bottom Hint */}
                    <div className="pt-3 border-t border-slate-800 text-center flex items-center justify-center gap-2 text-xs font-semibold text-indigo-400/80 group-hover:text-indigo-300">
                  ...
                  {/* BACK SIDE */}
                  <div className="absolute w-full h-full backface-hidden bg-gradient-to-b from-indigo-950 via-slate-900 to-slate-900 border-2 border-purple-500/60 rounded-3xl shadow-2xl flex flex-col justify-between p-6 sm:p-8 rotate-y-180 overflow-hidden">
                    {/* Top Tag */}
                    ...
                    {/* Card Back Content */}
                    <div className="flex-1 flex items-center justify-center text-center py-4 px-2 overflow-y-auto custom-scrollbar">
                      <div className="text-base sm:text-xl font-medium text-purple-100 leading-relaxed">
                        <MarkdownMath content={activeCard.back || '(Chưa nhập nội dung mặt sau)'} />
                      </div>
                    </div>

                    {/* Bottom Hint */}
                    <div className="pt-3 border-t border-purple-500/20 text-center flex items-center justify-center gap-2 text-xs font-semibold text-purple-300/80">
                      <RotateCw className="w-3.5 h-3.5" />
          ...
          {/* Controls Bar */}
          <div className="flex items-center justify-between gap-2 sm:gap-4 mt-2 pt-3 border-t border-slate-800 shrink-0">
            <button
              onClick={handlePrev}
              disabled={currentIndex === 0}
              className="px-3 sm:px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed rounded-2xl font-bold text-xs sm:text-sm border border-slate-700 flex items-center gap-1 sm:gap-2 transition-all shadow shrink-0 active:scale-95"
            >
            ...
            <button
              onClick={handleToggleFlip}
              className="flex-1 max-w-[200px] py-2.5 px-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs sm:text-sm rounded-2xl shadow-lg shadow-indigo-600/30 border border-indigo-400/30 flex items-center justify-center gap-2 active:scale-95 transition-all"
            >
          ...
          {/* Carousel Thumbnail Dots / Jump list */}
          {displayCards.length > 1 && (
            <div className="flex items-center justify-center gap-1 mt-2 pt-2 overflow-x-auto custom-scrollbar max-w-full px-2">
