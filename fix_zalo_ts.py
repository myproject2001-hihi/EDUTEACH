...
# Replace handleSendViaZalo definition with empty
content = re.sub(r'const handleSendViaZalo = async \(.*?\) => \{.*?\n  \};', 'const handleSendViaZalo = async () => {};', content, flags=re.DOTALL)

# Replace handleSendQuestion entirely
...
content = re.sub(r'const handleSendQuestion = async \(\) => \{.*?(?=const handleImportFlashcards =)', new_func + '\n  ', content, flags=re.DOTALL)
